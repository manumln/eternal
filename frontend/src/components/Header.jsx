...
import { Avatar, Button, Tooltip, Input } from "@nextui-org/react";
...
const Header = () => {
  ...
  const navLinkClasses = (isActive) =>
    `p-3 rounded-full flex items-center justify-center transition-all duration-300 ease-in-out ${
      isActive ? "bg-gradient-to-r from-blue-500 to-teal-400 text-white shadow-md" : "hover:text-white hover:bg-gray-700"
    }`;
      ...
      <div className="container mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <NavLink to={`/users/${userId}`} className="pb-0">
            <Avatar className="w-12 h-12 mx-auto rounded-full shadow-lg border-2 border-gray-200" src={profileImage} alt="user avatar" />
          </NavLink>
          <motion.div className="flex items-center gap-3 text-2xl font-bold text-sky-500 hover:text-sky-500 transition-all" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
        ...
        <div className="flex items-center gap-4">
          ...
          {isLoggedIn ? (
            <Tooltip content="Log Out">
              <Button onClick={handleLogout} className="flex items-center gap-2 px-4 py-2 rounded-full bg-red-500 text-white hover:bg-red-600 transition-all">
            ...
            <Tooltip content="Log In">
              <Button onClick={() => navigate("/login")} className="flex items-center gap-2 px-4 py-2 rounded-full bg-green-500 text-white hover:bg-green-600 transition-all">
